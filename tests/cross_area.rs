...
use support::{
    ...
    unregister_spawned_shep_pid, CURRENT_PROTOCOL,
};
...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/shep-cross-area-test-{}-{nanos}",
        std::process::id()
    ))
}
...
fn spawn_server(config_home: &Path, runtime_dir: &Path, api_socket_path: &Path) -> SpawnedShep {
...
fn cross_area_detach_and_reattach_preserves_state() {
    ...
    // Use shep: create a workspace and write output into its pane.
...
fn cross_area_server_kill_then_restart_and_reconnect() {
    ...
    cleanup_spawned_shep(server2, base);
}
