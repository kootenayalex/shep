...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/shep-autodetect-test-{}-{nanos}",
        std::process::id()
    ))
}
...
fn spawn_server(
    ...
    fs::create_dir_all(config_home.join("shep")).unwrap();
    fs::create_dir_all(runtime_dir).unwrap();
    register_runtime_dir(runtime_dir);
    fs::write(config_home.join("shep/config.toml"), "onboarding = false\n").unwrap();
    ...
    cmd.env("SHEP_SOCKET_PATH", api_socket_path);
...
/// Spawn `shep` (no subcommand) — the auto-detect launch path.
fn spawn_shep_auto(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
...
/// Spawn `shep --no-session` — the monolithic escape hatch.
fn spawn_shep_no_session(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
) -> SpawnedShep {
...
/// Running `shep` with no server present starts a server
...
fn auto_detect_no_server_spawns_server_and_attaches() {
    ...
    let api_socket = runtime_dir.join("shep.sock");
    ...
    // Run `shep` (no subcommand) — should auto-detect, spawn server, attach as client.
    let shep = spawn_shep_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    let client_pid = shep.child.process_id().expect("client should have PID");
    assert!(
...
/// Running `shep` with a server already running attaches
...
fn auto_detect_server_running_attaches_directly() {
    ...
    // Run `shep` (no subcommand) — should detect the running server and attach.
    let client = spawn_shep_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    cleanup_spawned_shep(client, PathBuf::from("/nonexistent"));
    cleanup_spawned_shep(server, base);
}

/// Socket path resolution is consistent between server and client.
/// Both derive the client socket from the `SHEP_SOCKET_PATH` override,
...
fn auto_detect_socket_path_consistency() {
    ...
    // Run `shep` with custom socket paths.
    let shep = spawn_shep_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
...
/// `shep --no-session` bypasses server/client and runs
...
fn no_session_flag_runs_monolithically() {
    ...
    // Run `shep --no-session` — monolithic mode, no server/client.
    let shep = spawn_shep_no_session(&config_home, &runtime_dir, &api_socket);
    ...
    // there is only one shep process.
    let client_pid = shep.child.process_id().expect("should have PID");
    assert!(
        process_exists(client_pid),
...
fn cli_subcommands_work_through_server() {
    ...
    // Test `shep workspace list` through the server's API socket.
    ...
    // Test `shep pane list` through the server's API socket.
...
/// persists after the client exits, and a new `shep` can reattach.
#[test]
fn auto_detect_server_persists_and_reattaches() {
    ...
    // Run `shep` — auto-detect spawns server + attaches client.
    let mut client1 = spawn_shep_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    // Run `shep` again — should detect the running server and reattach.
    let client2 = spawn_shep_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    thread::sleep(Duration::from_millis(500));
    ...
    cleanup_spawned_shep(client2, PathBuf::from("/nonexistent"));
    cleanup_test_base(&base);
}
...
fn auto_detect_default_socket_path_from_config_dir() {
    ...
    // Don't set SHEP_SOCKET_PATH or SHEP_CLIENT_SOCKET_PATH.
    ...
    let api_socket = config_home.join(app_dir_name).join("shep.sock");
    let client_socket = config_home.join(app_dir_name).join("shep-client.sock");
    ...
    cmd.env_remove("SHEP_SOCKET_PATH");
...
fn auto_detect_writes_client_and_server_logs_to_separate_files() {
    ...
    let spawned = spawn_shep_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    let client_log = log_dir.join("shep-client.log");
    let server_log = log_dir.join("shep-server.log");
    let monolith_log = log_dir.join("shep.log");
    ...
    assert!(
        ...
        "persistent client logs should not land in shep.log: {monolith_content}"
    );

    cleanup_spawned_shep(spawned, base);
}
...
fn no_session_writes_startup_logs_to_monolith_file() {
    ...
    let spawned = spawn_shep_no_session(&config_home, &runtime_dir, &api_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
...
fn auto_detect_respects_nested_guard_before_auto_attach() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_shep"))
        ...
        .env("SHEP_SOCKET_PATH", &api_socket)
