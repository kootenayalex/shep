...
use support::{
    cleanup_test_base, client_handshake, register_runtime_dir, register_spawned_shep_pid,
    send_input, unregister_spawned_shep_pid, wait_for_disconnect, wait_for_socket,
};
...
fn spawn_server(config_home: &Path, runtime_dir: &Path, api_socket: &Path) -> SpawnedShep {
...
fn spawn_server_with_env(
    ...
    cmd.env("SHEP_SOCKET_PATH", api_socket);
    cmd.env(
        "SHEP_CLIENT_SOCKET_PATH",
        runtime_dir.join("shep-client.sock"),
    );
...
fn spawn_named_session_server(
    ...
    fs::create_dir_all(config_home.join("shep-dev")).unwrap();
    fs::create_dir_all(runtime_dir).unwrap();
    fs::write(
        config_home.join("shep-dev/config.toml"),
        "onboarding = false\n",
    )
    .unwrap();
    ...
    cmd.env("XDG_CONFIG_HOME", config_home);
    cmd.env("XDG_RUNTIME_DIR", runtime_dir);
    cmd.env("SHEP_SESSION", session_name);
...
fn spawn_default_session_server(config_home: &Path, runtime_dir: &Path) -> SpawnedShep {
    fs::create_dir_all(config_home.join("shep-dev")).unwrap();
    fs::create_dir_all(runtime_dir).unwrap();
    fs::write(
        config_home.join("shep-dev/config.toml"),
...
fn wait_for_replacement_server_pid(runtime_dir: &Path, old_pid: u32, timeout: Duration) -> u32 {
    ...
    while Instant::now() < deadline {
        last_pids = support::shep_server_pids_for_runtime_dir(runtime_dir).unwrap_or_default();
...
fn wait_for_replacement_server_pid(_runtime_dir: &Path, old_pid: u32, timeout: Duration) -> u32 {
    let handoff_socket_pattern = format!("shep-handoff-{old_pid}.sock");
    let deadline = Instant::now() + timeout;
...
fn live_handoff_preserves_named_session_socket_paths() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let session_dir = config_home.join("shep-dev/sessions/work");
    let api_socket = session_dir.join("shep.sock");
    let client_socket = session_dir.join("shep-client.sock");
    ...
    assert!(
        !config_home.join("shep-dev/shep.sock").exists(),
...
fn live_handoff_accepts_canonical_pane_id_from_child_env() {
    ...
    assert_ok(request(
        ...
        serde_json::json!({
            ...
            "method": "pane.send_input",
            "params": {"pane_id": pane_id, "text": format!("printf '%s' \"$SHEP_PANE_ID\" > {}", pane_id_marker.display()), "keys": ["Enter"]}
        }),
    ));
...
fn live_handoff_preserves_http_servers_across_multiple_sessions() {
    ...
    let runtime_dir = base.join("runtime");
    let sessions = [
        (None, config_home.join("shep-dev/shep.sock")),
        (
            Some("work"),
            config_home.join("shep-dev/sessions/work/shep.sock"),
        ),
    ];
...
fn live_handoff_import_failure_rolls_back_old_server_at(failure_point: &str) {
    ...
    let spawned = spawn_server_with_env(
        ...
        &runtime_dir,
        &api_socket,
        &[("SHEP_TEST_HANDOFF_IMPORT_FAIL", failure_point)],
    );
    wait_for_socket(&api_socket, Duration::from_secs(10));
    register_runtime_dir(&runtime_dir);
