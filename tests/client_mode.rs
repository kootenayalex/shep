...
use support::{
    ...
    register_runtime_dir, register_spawned_shep_pid, unregister_spawned_shep_pid,
...
fn server_unreachable_shows_clear_error() {
    ...
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_shep"))
        .arg("client")
        .env("SHEP_DISABLE_SOUND", "1")
    ...
    assert!(
        stderr.contains("Is shep server running?"),
        ...
        while Instant::now() < deadline {
            match thin_reader.read(&mut buf) {
                Ok(n) if n > 0 => {
                    ...
                    if output.to_lowercase().contains("shep:") {
                        break;
                    }
                }
...
fn client_receives_notify_on_agent_state_change() {
    ...
    fs::create_dir_all(config_home.join("shep")).unwrap();
    fs::write(
        config_home.join("shep/config.toml"),
        "onboarding = false\n[ui.toast]\nenabled = true\n[ui.sound]\nenabled = true\n",
    )
    .unwrap();
    fs::create_dir_all(&runtime_dir).unwrap();
