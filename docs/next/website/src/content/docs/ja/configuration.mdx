...
組み込みテーマを選びます。既定は `shep` です。暖かいグラファイトの下地に銅色の
アクセントを合わせたもので、各色がちょうど 1 つの意味だけを担うように作られてい
ます: 赤はブロック、黄は実行中、青は完了で未確認、緑は落ち着いた状態、ティール
はキュー済みの入力、モーブはレビュー、銅色はフォーカスのみです。
