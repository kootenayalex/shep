...
| Pane | `pane.split`, `pane.swap`, `pane.move`, `pane.zoom`, `pane.layout`, `pane.process_info`, `pane.neighbor`, `pane.edges`, `pane.focus_direction`, `pane.resize`, `pane.list`, `pane.current`, `pane.get`, `pane.rename`, `pane.send_text`, `pane.send_keys`, `pane.send_input`, `pane.scroll`, `pane.read`, `pane.report_agent`, `pane.report_agent_session`, `pane.report_metadata`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.close`, `pane.mark_seen`, `pane.wait_for_output` |
| Layout | `layout.export`, `layout.apply`, `layout.set_split_ratio` |
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send`, `agent.rename`, `agent.focus`, `agent.start`, `agent.set_state`, `agent.clear_state` |
...
| Docket | `docket.list`, `docket.add`, `docket.update`, `docket.promote`, `docket.complete`, `docket.discard` |
| Overseer | `overseer.sample`, `overseer.chat`, `overseer.tick` |

`agent.start` takes either `argv` or a `runtime` name (`claude`, `opencode`, …); the name is resolved through the runtime manifest's `[launch]` recipe or a `[runtimes.<name>]` config table, and `runtime.list` reports every nameable runtime with `launchable`, `bin_resolved` and `headless` so a client can offer only what this server can start. See [Agents](/docs/agents/#launch-and-headless-recipes).
...
`overseer.sample` returns what the overseer plugin last sensed and said: its
narrative lines and who wrote them, the tick time, the ages of the situation and
the last brain answer, the health findings, the headless runtime the chat asks,
the tail of the chat (`chat_turns`, twenty by default, `0` for none) with the
total held, and the id of the conversation the chat shares with the overseer's
session pane. It creates nothing — a state dir that does not exist yet reads as
an overseer that has not spoken. `overseer.chat` puts one question to that
runtime and returns the recorded `you` turn; a second question while the first
is still out is refused with `overseer_chat_busy`, and the answer arrives as an
event rather than in the response. `overseer.tick` asks the plugin for a fresh
situation when the last one is older than `max_age_seconds` (60 by default, `0`
to tick whatever its age), reporting `invoked`, `in_flight` and the plugin
command `log_id`; a tick already running is reported, never stacked on.
...
`pane.scroll` scrolls a pane by a signed number of rows: positive goes back
into history, negative comes forward toward the present. Where the scroll goes
is the pane's business. A pane running a plain shell moves its own viewport
through terminal scrollback; a pane running a full-screen program has no
terminal scrollback at all — the alternate screen keeps none — so the scroll is
written into the pty as a wheel event and the program moves its own view. This
is the same routing a wheel over the pane gets in the TUI, and it is why the
method takes rows rather than a scroll offset.
...
`tab.move` reorders a tab (`insert_index`) or moves it, panes and all, into
another workspace (`workspace_id`) or a fresh one (`new_workspace: true`).
Every pane keeps its terminal and gets a new public id in the destination, as
with `pane.move`; the tab keeps its name. A cross-workspace move rebuilds the
tab as a row of right splits (pane identity is what a move preserves, not
ratios), refuses a zoomed tab, and closes the source workspace when the tab
was its last. `insert_index` is optional for a cross-workspace move and
defaults to the end. The response is the destination's `tab_list`.

```json
{"id":"req_tab_reorder","method":"tab.move","params":{"tab_id":"w1:t2","insert_index":0}}
{"id":"req_tab_regroup","method":"tab.move","params":{"tab_id":"w1:t2","workspace_id":"w3"}}
{"id":"req_tab_own_group","method":"tab.move","params":{"tab_id":"w1:t2","new_workspace":true}}
```
...
`workspace_id`, the `insert_index` the tab landed at, and updated ordered
`tabs` list for that workspace. A tab moved into another workspace gets a new
`tab_id` there, so the event's `tab_id` is the one to keep.
...
Overseer event subscriptions include `overseer.chat_turn` and
`overseer.updated`. `overseer.chat_turn` carries one recorded chat `turn` and
whether a question is still `pending`, so a client sees the question go out and
the answer come back without polling. `overseer.updated` fires when the
overseer's files move under the server — a tick landing, or a poll finding new
ones — and carries `tick_at`, the narrative `source`, and
`situation_age_seconds`; read the new state with `overseer.sample`. Neither is a
plugin hook event.
