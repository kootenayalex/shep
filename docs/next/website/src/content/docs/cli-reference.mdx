...
The **profile** is the boundary, not a prompt. `--profile overseer` (the default) serves reading and paging; `read` serves only the reading tools; `all` serves everything. A tool outside the profile is absent from `tools/list` and refused on call. A tool inside it can be narrower than its name: without the `docket` group `docket_add` forces `kind = "captured"` and `status = "inbox"` and drops `due`/`repeat` (keeping `source` and `notes`), and without the `send` group `agent_send` forces `queue = true`. `--allow` and `--deny` take comma-separated group or tool names and apply after the profile; `--tools` replaces the profile outright. `overseer_chat` and `overseer_tick` exist but are in no default profile — the overseer calling them would resume its own conversation or nest its own ticks.
