...
```bash
herdr doctor          # one pass over the server, socket, launchd, bridge, hooks, push and state
herdr doctor --json   # [{level, check, detail, fix?}]
```

`herdr doctor` prints one `ok|warn|fail <check>: <detail>` line per probe and
a `fix:` line under anything that is not ok. It exits 1 only when a probe
fails (the server is down, the socket file is stale, the bridge address has
no listener, the docket store will not open); warnings are advice and exit 0.
...
herdr agent set-state <target> <idle|working|blocked|unknown|custom-name>
herdr agent clear-state <target>
...
herdr agent start <name> [...] --runtime <runtime>
herdr agent explain <target> [--json|--verbose]
herdr agent explain --file PATH --agent LABEL [--json|--verbose]
```

`agent start --runtime claude` starts a runtime by name instead of spelling out argv: the manifest's `[launch]` recipe (or a `[runtimes.claude]` table in `config.toml`) says what to run. `--runtime` and `-- argv` are mutually exclusive. See [Runtimes](#runtimes).
...
`agent set-state` pins a state on the agent by hand: one of the built-in states, or the `name` of a `[[states.custom]]` entry from the config. The override shows on every surface with a `·` marker and holds until you run `agent clear-state`, or until the agent's detected state changes on its own — a real transition is new evidence and wins.
...
```bash
shep runtime list [--json]
shep runtime ask <runtime> <prompt|-> [--timeout SECS] [--cwd PATH] [--mcp-profile NAME]
```

`runtime list` asks the running server which runtimes it can name — every bundled detection manifest plus any `[runtimes.<name>]` table in `config.toml` — and, for each, whether it is `launchable` (a `[launch]` recipe exists and its program is on the server's `PATH`, with the resolved path) and whether it has a `headless` recipe.

`runtime ask` runs the runtime's `[headless]` recipe once with the prompt on stdin (or as the last argument, when the recipe says `prompt = "arg"`), streams the runtime's stdout and stderr through, and exits with the runtime's exit code. `-` reads the prompt from stdin. `--timeout` (default 120 seconds) kills the runtime and exits 124 when it does not answer in time. Exit code 2 means the runtime is unknown (`runtime_unknown`) or has no headless recipe (`runtime_not_launchable`). It runs locally, so it needs no server. `--mcp-profile NAME` writes that profile's MCP client config to `<state dir>/mcp/NAME.json` and attaches it through the recipe's `mcp_config_args`, so the runtime can answer with shep's own tools; a runtime whose recipe does not declare them is asked without tools and says so on stderr. `ask` never passes session arguments — one question, its own conversation.

```bash
shep mcp [serve] [--socket PATH] [--profile NAME] [--allow a,b] [--deny a,b] [--tools a,b]
shep mcp config [--profile NAME] [--output PATH]
shep mcp tools [--profile NAME]
```

`shep mcp` serves the running session to an MCP client as tools over stdio: newline-delimited JSON-RPC on stdin and stdout, nothing else on stdout. `serve` is the default verb. Most tools go to the running server over its socket (`session_overview`, `agent_get`, `agent_read`, `docket_*`, `agent_send`, `overseer_*`); `pane_todos`, `pane_transcript`, `memory_search`, `memory_show` and `push_page` are answered locally the way the bridge answers them; `doctor` runs in the same process and returns the findings `shep doctor --json` prints.

The **profile** is the boundary, not a prompt. `--profile overseer` (the default) serves reading, inbox capture and paging; `read` serves only the reading tools; `all` serves everything. A tool outside the profile is absent from `tools/list` and refused on call. A tool inside it can be narrower than its name: without the `docket` group `docket_add` forces `kind = "captured"` and `status = "inbox"` and drops `due`/`repeat` (keeping `source` and `notes`), and without the `send` group `agent_send` forces `queue = true`. `--allow` and `--deny` take comma-separated group or tool names and apply after the profile; `--tools` replaces the profile outright. `overseer_chat` and `overseer_tick` exist but are in no default profile — the overseer calling them would resume its own conversation or nest its own ticks.

`shep mcp tools` prints the permitted tool names, one per line. `shep mcp config` prints the `mcpServers` block a client needs, naming this executable and the chosen profile (and `--socket` when `SHEP_SOCKET_PATH` is set); `--output PATH` writes it instead. Groups: `read`, `docket-inbox`, `docket`, `nudge`, `send`, `mark`, `seen`, `push`, `overseer`, `tick`.
...
The overseer plugin's own read of the room is on the same socket:

```bash
shep overseer sample [--chat N] [--json]
shep overseer chat <text|-> [--wait [--timeout SECS]] [--json]
shep overseer tick [--max-age SECS] [--json]
```

`overseer sample` prints one compact board: the `✦ read of the room` narrative, a `tick hh:mm · brain Nm ago · <runtime> headless` line, the health findings (`✓` ok, `⚠` warning, `◉` failure), the tail of the chat (`--chat N` trailing turns, default 20, `0` for none) and the shared session id. It says `plugin not linked`, `no runtime` and `session not started` plainly when that is the state, and it creates nothing — not the state dir, not the session id. The docket is deliberately not part of it; that is `shep docket list`.

`overseer chat` puts one question to the overseer's headless runtime and prints the `you` turn it recorded. `-` reads the question from stdin. `--wait` subscribes to `overseer.chat_turn` before sending and then prints the answer alone, so it can be piped; it exits 1 on `--timeout` (default 120 seconds, matching the server). A second question while one is still out is refused with `overseer_chat_busy`.

`overseer tick` asks the plugin for a fresh situation and prints `tick invoked (log <id>)`, `tick skipped: situation is Ns old`, or `tick already in flight`. `--max-age` (default 60 seconds) is the age below which the situation is still considered current; `0` ticks whatever its age.

`--json` on any of the three prints the raw `result` object; an error prints the whole response on stderr and exits 1.
