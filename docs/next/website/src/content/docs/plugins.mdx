...
session_argv = ["claude"]                  # what the board's session button runs
session_cwd = "~/vault/agents/shep-overseer"  # where; default: the plugin's state dir
```
...
`session` pane: `session_argv` (default `["claude"]`; the server's
`SHEP_OVERSEER_SESSION_ARGV` overrides it) in `session_cwd` when that
directory exists, else the plugin's state dir, with `SHEP_OVERSEER_STATE_DIR`,
