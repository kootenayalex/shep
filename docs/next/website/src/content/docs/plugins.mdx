...
`shep runtime ask <runtime>` and asks for a board and a JSON list of proposed
inbox items; the board replaces the deterministic one and each proposal whose
`source` is new becomes `shep docket add … --kind captured`, inbox only, at
most five per tick. The plugin never sends keys or text to a pane, never
touches the server process, never nudges, and never promotes or dates a docket
item; `tests/plugin_overseer.rs` greps its script for the forbidden verbs. See
...
now rest on: reading, `docket_add` into the inbox, and paging the phone. There
is no tool that types into a pane, queues, restarts or kills, so the rules
