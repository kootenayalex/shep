...
`situation.md` in the plugin's state dir and a `BOARD.md` of at most 40 lines
— blocked agents first, every agent with its state age, health fails and
warns, docket due and inbox. With nothing configured that deterministic board
is all it does. Name a brain and it thinks too:
