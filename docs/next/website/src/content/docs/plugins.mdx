...
`SHEP_PLUGIN_CONFIG_JSON` carries the plugin's own `[plugins.<id>]` table from
`config.toml` as one JSON object (`{}` when unset), for commands and panes
alike, so a plugin's settings can live in the one config file a person already
edits and reload with it. shep never interprets the table; the plugin owns its
keys. See [Configuration](/docs/configuration/#plugins).
...
shep ships one plugin in its checkout, `plugins/overseer/`: a personal
assistant that narrates the session and proposes docket items, and never acts
for you. Link it from a checkout and open its pane:

```bash
shep plugin link plugins/overseer
shep plugin pane open --plugin overseer --entrypoint board
```

Its two event hooks (`pane.agent_status_changed`, `pane.exited`) run one tick:
`shep doctor --json`, `session.overview`, `shep docket list --json` become
`situation.md` in the plugin's state dir and a `BOARD.md` of at most 10 lines
— a header and a short read of the room in prose: who is blocked and waiting
on you first, then what the docket owes, then health. The overseer board in
the TUI (`ctrl+alt+b`) draws the agent table, the docket and the health strip
around that narrative. With nothing configured that deterministic board is all
it does. Name a brain and it thinks too:

```toml
[plugins.overseer]
runtime = "claude"     # any runtime `shep runtime list` marks headless
session_argv = ["claude", "--resume", "{session_id}"]  # optional; the default resumes the board's conversation
session_cwd = "~/vault/agents/shep-overseer"  # where the chat and the pane run; default: the plugin's state dir
```

At most once every ten minutes the tick hands the situation to
`shep runtime ask <runtime>` and asks for one board: a `## <agent>` section per
agent, blocked first, then `## room` for what cuts across them; it replaces the
deterministic board, which has the same shape, and the desktop and the phone
seat each section under its agent's row. The plugin never sends keys or text
to a pane, never touches the server process, never nudges, and never writes to
the docket — that list is yours; `tests/plugin_overseer.rs` greps its script
for the forbidden verbs. See `plugins/overseer/README.md`.

The board's `▸ open the overseer's session` button runs the plugin's
`session` pane, and that pane is the board's chat in another face: shep
hands it `SHEP_OVERSEER_SESSION_ID` (the shared conversation's id, minted
into `session-id` in the state dir by the first chat line or the first
open), `SHEP_OVERSEER_SESSION_RESUME` (`1` once something has begun it) and
`SHEP_OVERSEER_SESSION_CWD` (`session_cwd` when that directory exists, else
the plugin's state dir — the chat runs there too). `overseer-session` runs
`session_argv` (or the server's `SHEP_OVERSEER_SESSION_ARGV`) with
`{session_id}` substituted, else `claude --resume <id>` /
`claude --session-id <id>`, with `SHEP_OVERSEER_STATE_DIR`,
`SHEP_OVERSEER_SITUATION`, `SHEP_OVERSEER_BOARD` and `SHEP_OVERSEER_CHAT`
exported. shep keeps that session in a system workspace no list shows.

All three faces of the overseer — the tick's brain, the board's chat and that
session — reach shep through `shep mcp`, when the runtime's recipe says how to
attach a client config (`mcp_config_args`; the bundled `claude` manifest does).
shep writes `mcp.json` into the plugin's state dir and hands it over: the tick
adds `--mcp-profile overseer` to its `shep runtime ask`, the chat passes the
same file, and the session pane gets `SHEP_OVERSEER_MCP_CONFIG` with
`SHEP_OVERSEER_MCP_ARGS` (the launch recipe's words, already substituted) —
appended to the default argv, or placed by a `session_argv` that names
`{mcp_config}` itself. The `overseer` profile is the boundary the hard rules
now rest on: reading and paging the phone. There is no tool that types into a
pane, queues, restarts, kills or writes to the docket, so the rules
hold even when the prose is forgotten, and `[plugins.overseer] tools` is where
