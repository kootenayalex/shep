...
The board has one detail screen. `i` opens the selected agent: its state, status, activity, working directory, and context gauge, plus a live window onto what the pane is showing — everything the card has to truncate, without attaching to it. Esc steps back to the columns before it closes the board.
