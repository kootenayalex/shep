...
The session board is the full-screen agent overview: `blocked`, `done`, `working`, and `idle` columns in descending attention order. Two options make it the screen you start and return to.
...
`open_on_board` starts a session on the board instead of dropping straight into a pane, so a session with several agents opens on the overview. Enter focuses the selected pane. The board and the desktop are two sides of one screen: `keys.switch_view` (`ctrl+alt+b` by default) flips between them from anywhere, and `keys.board` is an optional second binding that only opens it.

The board is two lane boards one key apart, and `board_view` picks which one it opens on. `docket` (the default) is the personal docket — inbox, due, slated, recurring, done — one card per item, with the verbs on keys: `n` captures a new item into the inbox, `p` and `r` promote the selected inbox item as slated or weekly-recurring, `d` completes it (a recurring item rolls its date forward), `x` discards it, and `i` opens the item in full. `agents` is the agent lanes, one per group. `a` flips between the two, and the dashboard strip on either counts what the docket has due and waiting.

Each lane board has a detail screen. `i` opens the selected agent: its state, status, activity, working directory, and context gauge, plus a live window onto what the pane is showing — everything the card has to truncate, without attaching to it. On the docket it opens the item: every field and the notes whole. Esc steps back to the lanes before it closes the board.
