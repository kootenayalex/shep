...
`titlebar` and `hint_bar` are the desktop chrome: the top row reads `shep · group › agent`, a tally of agents by state (`◉ 1  ⠹ 2  ● 1  ○ 1`) and the `desktop | board` pill; the bottom row names the prefix chord, the board chord, `?` and `q`. On a narrow terminal the tally drops idle, then done, then shortens the pill, then drops working and blocked — always a prefix of that order, never a gap. Both are on by default.
