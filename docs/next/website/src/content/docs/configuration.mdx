...
The board has two detail screens. `i` opens the selected agent: its state, status, activity, working directory, and context gauge, plus a live window onto what the pane is showing — everything the card has to truncate, without attaching to it. `t` opens the task queue behind the dashboard's task count, listing each task's state, repo, and runtime. Esc steps back to the columns from either screen before it closes the board.
...
This trade needs a host terminal that disambiguates escape codes, which means Ghostty, kitty, or WezTerm. Without one, `shift+esc` arrives as a bare Esc and the agent can no longer be interrupted from the keyboard; set `escape_returns_to_board = false` on those hosts and use Esc for the interrupt as before.
