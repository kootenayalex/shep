...
- Companion hardening and live input. The bridge pairing (URL + token) now lives in an `EncryptedSharedPreferences` file keyed by the Android keystore, migrated out of plain preferences on first read; backups and device transfers are off; `BridgeClient` refuses to send the token over `ws://` to any address that is not loopback, the emulator host, RFC 1918, the tailnet's CGNAT block, link-local, IPv6 ULA/link-local or a local-only name; `assembleRelease` fails without a real keystore instead of signing with the debug key. Keys typed before a pane's stream is up, or after it ended, now go as `pane.send_text` / `pane.send_keys` requests in order instead of hitting an empty handler; a refused stream write is reported; Gboard delete bursts are one `pane.send_keys` write with N backspaces; the keyboard is put away when input leaves stream mode or the pane screen goes; switching back from the recorded view no longer forces stream input, and an input mode picked by hand survives toggling the output. In stream mode the input view holds focus from the start, so a hardware keyboard reaches the pty without a tap on the grid (the soft keyboard still rises only on that tap). The pairing screen has a clear button on the token field, and a token the bridge refused is no longer kept as the saved pairing. Sheets open fully expanded (and the new-task sheet scrolls), so the "queue task" button is on screen on a phone and back dismisses the sheet in one press; the app declares protocol 17 to match the server. Two dead files (`YouScreen.kt`, `AnsiRender.kt`) are gone.
- Push notifications are now one per agent, and they clear themselves. `shep bridge notify-push` sends a `tag` (the pane id) with every message so a newer event for the same agent replaces the older notification instead of stacking beside it, and an `op` of `show` or `clear`: the server fires a clear (`SHEP_NOTIFY_OP=clear`) once a notified pane is looked at — on the active tab of a focused desktop client, through `pane.mark_seen` from a companion, or because the pane closed — so a question answered at the desk stops paging the phone. The Android companion cancels the notification on `clear`, understands all seven server kinds (`idle`, `working`, `blocked`, `unknown`, `done`, `task`, `review`), and calls `pane.mark_seen` whenever an agent is opened. The self-hosted ntfy/UnifiedPush transport is gone from the companion (FCM was the transport that worked); a stale `unifiedpush` row in `<config>/push-endpoints.json` can be removed by hand, it is harmless but never delivers. Exec commands that predate this see `SHEP_NOTIFY_OP` as a new variable and should treat an absent one as `show`. `SHEP_NOTIFY_PANE_ID` now carries the pane's public id (`w2:p1`) instead of the raw pane number, which no `pane.*` method could resolve; the companion's notification tap and approve/deny actions depend on it.
- Manual and custom agent states. `agent.set_state {target, state?|custom?}` and `agent.clear_state {target}` (CLI `shep agent set-state` / `clear-state`; TUI `prefix+shift+s` or the pane context menu's "Set state..."; the companion's channel sheet) pin a state by hand. The override holds until cleared or until the detected state changes on its own, never fires a notification, and survives snapshots and live handoff. It does drive tab-to-queue both ways: a manual `working` holds queued prompts back and a manual `idle` releases them, the same as detected ones. Custom states come from config:
  ```toml
  [[states.custom]]
...
- Added Android companion review & ship (A5): new `workspace.diff` (target ref + `--stat` + capped unified diff) and `workspace.ship` (merge a linked worktree's branch into its base, refusing on dirty/detached/conflict) JSON API methods, so the companion can review a diff, request changes (feedback into the agent pane + `workspace.set_review_state`), and ship — then clean up with `worktree.remove`.
...
- Added `pane.scroll` (`shep pane scroll <pane_id> <rows>`): scroll a pane by a signed number of rows, routed the way a wheel over that pane is routed — terminal scrollback for a pane that has any, a forwarded wheel event for a full-screen program that keeps its own view. Remote clients could previously read a pane's history but not scroll it, and for an agent on the alternate screen there was no history to read.
- `session.overview` agents now carry `activity_lines`, the last few lines of real content on the pane's screen in reading order, beside the single `activity_line` (which stays the last of them). A client with room for more than one line — the phone's agent list — can show what an agent is doing rather than only the last thing it printed.
...
- Session-board cards now show the shortest name no other agent in the session answers to, and `session.overview` publishes it as `display_name`. Five claude sessions in the same repo were five cards reading "claude" — true, and useless — but spending detail on every card is no better, so an agent that is already the only "claude" stays "claude" and only the ones that collide grow a workspace, then a branch, then a location, with the pane id as a last resort. Deciding this on the shared board model rather than per client is what stops the desktop and the companion calling the same agent different things.
- `shep bridge notify-push` now delivers over FCM as well as UnifiedPush, and each registered device carries the notification kinds it wants. FCM is what actually wakes an Android app out of Doze; the message is data-only so the companion still builds its own notification and keeps its Approve/Deny actions. The service-account key goes at `<config>/fcm-service-account.json`, access tokens are minted by signing a JWT with `openssl` and cached for the hour they are valid, and a token FCM reports as unregistered is dropped rather than retried forever. New bridge-local `push.set_kinds` (change one device's subscriptions without re-pairing) and `push.test` (send a real notification now and report per-device what happened) — push failing is otherwise completely invisible, which is how a broken setup can go unnoticed indefinitely. `SHEP_NTFY_PUBLISH_BASE` now applies only to UnifiedPush rows; it previously rewrote every publish target, which silently redirected notifications away from the device that registered for them.
...
- `shep bridge` now relays only an allowlist of API methods to a paired companion (the verbs the app has UI for: agents, panes, tabs, workspaces, worktrees, `task.dispatch`, `events.subscribe`); everything else is answered with `method not allowed over the bridge`. The `?token=` query-string fallback is gone — send `Authorization: Bearer <token>` — the token is compared in constant time, upgrades carrying an `Origin` header (browser pages) are refused with 403, at most 16 clients are served at once, and each failed authentication delays that address's next handshake. `memory.*` over the bridge now refuses a `repo` path that does not resolve under the home directory. `scripts/test_bridge_allowlist.py` keeps the allowlist and the companion's calls in step.
- A tab and the agent in it are now one name, not two. A tab holding a single pane shows that agent's name everywhere the tab is named — the tab bar, the top chrome, the navigator, the mobile switcher, the sidebar, the board and the phone — and renaming it writes the agent's name whichever verb you reach for: `rename_tab` in the TUI now opens the agent's rename prompt, and `tab.rename` on a one-agent tab sets the agent's name rather than a second name of its own. `pane.rename` sets the agent's name too, so the pane border cannot disagree with the panel above it. A tab's own name is still stored and still answers for a tab that has no named agent, or that really holds several panes and is a container again.

### Fixed
- `shep server live-handoff` no longer shrinks every imported pane to 80×24. The new server rendered its first frame before any client attached, and that frame is the one that sizes freshly imported panes, so it used the no-client floor size and every agent reflowed to 80 columns until someone opened the TUI. The exporting server now records its last foreground terminal size in the handoff manifest and the importer renders at that size (or the largest imported pane, for manifests from older servers) until a client attaches.
- A pane's reported activity is no longer the agent's own hint bar. The bottom-most line with letters in it is almost always chrome, so every claude pane reported `bypass permissions on (shift+tab to cycle)` — the input box and everything below it is now cut before the lines are taken, and a label baked into that box's top border (`───── clear-chat-history ──`) no longer reads as content either.
- `shift+tab` sent through the socket API (`pane.send_keys`, `pane.send_input`, `agent.send`, and so the Android companion) now reaches the agent as CSI Z instead of an ordinary tab. The combo canonicalises to a bare `BackTab` for binding lookup, where the shift is implicit in the code, but the pty encoder reads the modifier set — so every API caller pressing shift+tab was silently pressing tab, and cycling claude's permission modes from a phone did nothing.
- Agent state reported through `pane.report_agent` now advances the task queue and the review gate, as output-detected state already did. Only the two output-driven paths ran the task bookkeeping, so a runtime that reports its own state through lifecycle hooks — the most reliable kind — left its task stuck at `running` forever and never marked its workspace `needs_review`.
- `agent.rename` no longer reports `agent_not_found` when clearing the name of a pane that has no detected agent. A plain shell is an agent only because someone named it, so clearing that name un-agents it and the lookup used to build the response came back empty — the rename had already succeeded, and the caller was told it failed. Clearing now also drops the manual pane label that naming set, so a reset actually resets both halves instead of leaving the old name on the pane.
...
- A prompt sent with `agent.send` (or queued from the TUI's queue-prompt modal, or sent as review feedback) no longer sometimes lands as a blank line in the agent's input box instead of being submitted. The text and its Enter went out as one buffer, and readline libraries read a burst of bytes ending in a carriage return as *pasted* content — so the Enter became a newline in the input rather than the keypress that runs it. The text now goes out as a bracketed paste when the pane negotiated one, with the Enter following as its own write.
...
- macOS installs are now code-signed with a stable identity (`just install-macos`), so the system stops re-asking for file, photo, and app-data access after every rebuild. macOS keys privacy grants to the code signature, and cargo's ad-hoc linker signature has no stable identity — TCC fell back to the content hash, which changes on every build, so each rebuild looked like a new program and silently discarded every grant. See docs/BUILD-macos.md.
...
- The sidebar and the board no longer say a group's name twice. An agent named after its own group — `ShiftMayt Legal` in the `ShiftMayt` workspace — now renders as `Legal`, and a name that only repeats the workspace or tab already on the line above it is left off entirely. Only the drawing changes; the stored name keeps whatever was typed, so the phone still shows it in full.
