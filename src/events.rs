...
pub enum AppEvent {
    ...
    /// The last lines of real screen content for a pane, in reading order.
    /// Like `AgentContextReported` this is a decoupled display hint: it never
    /// drives notifications or detection, only the `activity_lines` runtime
    ...
    /// re-emits it through shep's own clipboard writer.
