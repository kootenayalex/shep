...
///
/// Contractual override behavior for auto-detect uses `SHEP_SOCKET_PATH`.
/// This variable is kept as a fallback for callers that explicitly need a
/// client-only override when `SHEP_SOCKET_PATH` is not set.
pub const CLIENT_SOCKET_PATH_ENV_VAR: &str = "SHEP_CLIENT_SOCKET_PATH";
...
/// 2. If `SHEP_SOCKET_PATH` is set, derive the client socket path from it by
///    inserting `-client` before `.sock` (e.g. `shep.sock` -> `shep-client.sock`).
///    This keeps JSON API and client socket overrides consistent.
/// 3. Otherwise, honor `SHEP_CLIENT_SOCKET_PATH` (legacy/testing fallback).
...
pub fn client_socket_path() -> PathBuf {
    ...
    client_socket_path_from_overrides(
        crate::env_compat::var(crate::api::SOCKET_PATH_ENV_VAR).as_deref(),
        crate::env_compat::var(CLIENT_SOCKET_PATH_ENV_VAR).as_deref(),
    )
}
...
pub(crate) fn prepare_socket_path(path: &Path) -> io::Result<()> {
    crate::ipc::prepare_socket_path(path, |path| {
        format!(
            "shep server is already running (socket busy at {})",
            path.display()
        )
    })
}
...
mod tests {
    ...
    fn client_socket_path_derived_from_api_socket_override() {
        let path = client_socket_path_from_overrides(Some("/tmp/test-shep.sock"), None);
        assert_eq!(path, PathBuf::from("/tmp/test-shep-client.sock"));
    }
    ...
    fn client_socket_path_respects_legacy_client_override_without_api_override() {
        let path = client_socket_path_from_overrides(None, Some("/tmp/test-shep-client.sock"));
        assert_eq!(path, PathBuf::from("/tmp/test-shep-client.sock"));
    }
    ...
    fn client_socket_path_defaults_to_config_dir() {
        ...
        assert_eq!(path, crate::config::config_dir().join("shep-client.sock"));
    }
