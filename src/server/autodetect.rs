//! Auto-detect launch behavior for the `shep` command.
//!
//! When the user runs `shep` with no subcommand:
...
/// directory where the user ran `shep`.
pub(crate) const STARTUP_CWD_ENV_VAR: &str = "SHEP_STARTUP_CWD";

// ---------------------------------------------------------------------------
// Server detection
// ---------------------------------------------------------------------------

/// Checks whether a shep server is currently listening on the client socket.
///
...
/// Checks whether a shep server is listening at a specific socket path.
...
fn validate_running_server_compatibility() -> io::Result<()> {
    let Some(status) = read_server_status()? else {
        return Err(io::Error::other(format!(
            "a shep server is listening, but its status API is unavailable.\n\n{}\nIf that fails, stop the old server process manually.",
    ...
    Err(io::Error::other(format!(
        "Shep was updated, but this session is still running the old server.\n\nserver: v{} protocol {}\nclient: v{} protocol {}\n\n{}",
...
/// Spawns the shep server as a background daemon process.
///
...
/// - Inherits relevant environment variables (`XDG_CONFIG_HOME`, `SHEP_SESSION`,
...
pub fn spawn_server_daemon() -> io::Result<u32> {
    let exe = std::env::current_exe().map_err(|err| {
        io::Error::new(
            err.kind(),
            format!("failed to determine shep executable path: {err}"),
        )
    })?;
    ...
    let child = command.spawn().map_err(|err: io::Error| {
        io::Error::new(err.kind(), format!("failed to spawn shep server: {err}"))
    })?;
...
fn build_server_daemon_command(exe: PathBuf) -> Command {
    ...
    if crate::session::explicit_session_requested() {
        crate::env_compat::remove_std_child_env(&mut command, crate::api::SOCKET_PATH_ENV_VAR);
        crate::env_compat::remove_std_child_env(&mut command, "SHEP_CLIENT_SOCKET_PATH");
    }
...
pub fn wait_for_server_socket(socket_path: &Path, timeout: Duration) -> io::Result<()> {
    ...
    Err(io::Error::new(
        io::ErrorKind::TimedOut,
        format!(
            "server did not become ready within {}s (socket: {}). The background server may still be starting; try `shep` again, or check {}",
            timeout.as_secs(),
            socket_path.display(),
...
///
/// This is the entry point called from `main.rs` when the user runs `shep`
...
mod tests {
    ...
    fn server_daemon_command_clears_socket_overrides_for_explicit_session() {
        ...
        std::env::set_var("SHEP_CLIENT_SOCKET_PATH", "/tmp/inherited-client.sock");
        ...
        let command = build_server_daemon_command(PathBuf::from("/tmp/shep-test"));
        ...
        assert!(envs.iter().any(|(key, value)| {
            *key == OsStr::new("SHEP_CLIENT_SOCKET_PATH") && value.is_none()
        }));
        crate::env_compat::remove_process_env_for_test(crate::api::SOCKET_PATH_ENV_VAR);
        crate::env_compat::remove_process_env_for_test("SHEP_CLIENT_SOCKET_PATH");
        crate::env_compat::remove_process_env_for_test(crate::session::SESSION_ENV_VAR);
        crate::session::clear_explicit_session_for_test();
    }
...
"#,
    ...
    fn validate_running_server_compatibility_names_session_commands_for_protocol_mismatch() {
        ...
        assert!(
            message.contains("Run `shep session stop work`"),
            "unexpected error: {message}"
        );
        assert!(
            message.contains("then run `shep session attach work` again"),
            "unexpected error: {message}"
        );
