...
impl AppState {
    ...
    pub(super) fn workspace_at_row(&self, row: u16) -> Option<usize> {
        ...
        cards.iter().find_map(|card| {
            (card.is_group() && row >= card.rect.y && row < card.rect.y + card.rect.height)
    ...
    pub(super) fn workspace_drop_index_at_row(&self, row: u16) -> Option<usize> {
        ...
        let cards: Vec<_> = cards.into_iter().filter(|card| card.is_group()).collect();
        if cards.is_empty() {
            return Some(0);
        }
    ...
    pub(super) fn on_agent_panel_sort_toggle(&self, col: u16, row: u16) -> bool {
        ...
        let rect =
            crate::ui::sidebar_sort_toggle_rect(self.workspace_list_rect(), self.agent_panel_sort);
        rect.width > 0
            && col >= rect.x
            && col < rect.x + rect.width
            && row >= rect.y
    ...
    /// The agent a sidebar row stands for, if that row is an agent's.
...
#[cfg(test)]
mod tests {
    /// Where the tree draws this agent's own row.
    fn agent_card_for(
        ...
        crate::ui::compute_workspace_card_areas(state, state.view.sidebar_rect)
            .into_iter()
            .find(|card| card.agent().is_some_and(|(_, _, id)| id == pane_id))
            .map(|card| card.rect)
    }
    ...
    fn clicking_agent_detail_row_switches_to_correct_tab_and_pane() {
        ...
        let row = agent_card_for(&app.state, second_pane).expect("second agent has a row");
        app.handle_mouse(mouse(
            MouseEventKind::Down(MouseButton::Left),
            row.x + 2,
            row.y,
    ...
    fn clicking_agent_panel_toggle_switches_sort() {
        ...
        let toggle = crate::ui::sidebar_sort_toggle_rect(
    ...
    #[test]
    fn scrolling_the_tree_with_the_wheel_moves_the_one_list() {
        let mut app = app_for_mouse_test();
        let mut ws = Workspace::test_new("test");
        let first_pane = ws.tabs[0].root_pane;
        ...
        app.state.selected = 0;
        app.state.mode = Mode::Terminal;
        // Short enough that the group and its four agents cannot all fit.
        app.state.view.sidebar_rect = Rect::new(0, 0, 26, 7);

        let list_area = app.state.workspace_list_rect();
        assert!(crate::ui::should_show_scrollbar(
            crate::ui::workspace_list_scroll_metrics(&app.state, list_area)
        ));
    ...
    fn clicking_worktree_parent_row_focuses_workspace_without_toggling() {
        ...
        for (idx, checkout_path) in ["/repo/shep", "/repo/shep-issue"].into_iter().enumerate() {
    ...
    fn wheel_workspace_selection_follows_grouped_visual_order_without_scrollbar() {
        ...
        for (idx, checkout_path) in [(0, "/repo/shep"), (2, "/repo/shep-issue")] {
            app.state.workspaces[idx].worktree_space =
                Some(crate::workspace::WorktreeSpaceMembership {
                    key: "repo-key".into(),
                    label: "shep".into(),
        ...
        let labels: Vec<_> = app.state.workspaces[0]
            ...
            .map(|(tab_idx, _)| {
                app.state.workspaces[0]
                    .tab_display_name(tab_idx, &app.state.terminals)
                    .unwrap()
            })
            .collect();
