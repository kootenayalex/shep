                ...
                if in_sidebar {
                    ...
                    if self.on_agent_panel_sort_toggle(mouse.column, mouse.row) {
                        self.agent_panel_sort = match self.agent_panel_sort {
                            AgentPanelSort::Grouped => AgentPanelSort::Priority,
                            AgentPanelSort::Priority => AgentPanelSort::Grouped,
                        };
...
mod tests {
    ...
    #[tokio::test]
    async fn mouse_dispatcher_does_not_forward_motion_behind_shep_modes() {
        let mut app = app_for_mouse_test();
        let mut ws = Workspace::test_new("test");
        let pane_id = ws.tabs[0].root_pane;
        let (runtime, mut input_rx) =
    ...
    #[tokio::test]
    async fn unset_right_click_passthrough_keeps_modified_right_click_as_shep_menu() {
        let mut app = app_for_mouse_test();
        let mut ws = Workspace::test_new("test");
        let pane_id = ws.tabs[0].root_pane;
        let pane_infos = ws.tabs[0].layout.panes(Rect::new(26, 2, 80, 18));
