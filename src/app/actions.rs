...
impl AppState {
    ...
    pub fn handle_app_event(&mut self, event: AppEvent) -> Vec<PaneStateUpdate> {
        match event {
            ...
            AppEvent::AgentActivityReported { pane_id, lines } => {
                if let Some(terminal_id) = self.workspaces.iter().find_map(|ws| {
                    ws.pane_state(pane_id)
                        .map(|pane| pane.attached_terminal_id.clone())
                }) {
                    if let Some(terminal) = self.terminals.get_mut(&terminal_id) {
                        terminal.set_activity_lines(lines);
                    }
                }
                Vec::new()
            }
            ...
            AppEvent::OverseerChatFinished { .. } => Vec::new(),
        }
    }
...
mod tests {
    ...
    fn double_click_word_bounds_cover_terminal_text() {
        let cases = [
            ...
            (
                "open ../shep-worktrees/issue-1",
                "shep",
                "../shep-worktrees/issue-1",
            ),
            (
            ...
            ("cargo test --package=shep", "--package", "--package=shep"),
            (
            ...
            (
                "tmux|newhoo|fixhoo|newmoo|notification|window_bell|shep",
                "newhoo",
                "newhoo",
            ),
            (
    ...
    async fn navigator_rows_match_live_root_runtime_cwd_workspace_label() {
        ...
        state.navigator.query = "shep".into();
    ...
    fn navigator_search_only_matches_visible_row_text() {
        let mut state = app_with_workspaces(&["one"]);
        state.workspaces[0].identity_cwd = "/tmp/shep-worktrees/issue-work".into();
    ...
    fn terminal_cwd_report_updates_terminal_cwd_and_marks_session_dirty() {
        ...
        let terminal_id = state.workspaces[0]
            ...
            .attached_terminal_id
            .clone();
        let cwd = std::env::temp_dir().join(format!("shep-cwd-report-test-{}", std::process::id()));
    ...
    #[test]
    fn active_workspace_active_tab_keeps_shep_toast_suppressed_when_outer_terminal_is_unfocused() {
        let mut state = app_with_workspaces(&["active"]);
        state.active = Some(0);
        state.outer_terminal_focus = Some(false);
        state.toast_config.delivery = crate::config::ToastDelivery::Shep;
    ...
    fn update_ready_sets_manual_update_toast() {
        ...
        assert_eq!(
            toast.context,
            "detach, run `shep update`, then follow its restart guidance"
        );
    }
    ...
    fn update_ready_uses_event_install_command_in_toast() {
        ...
        state.handle_app_event(AppEvent::UpdateReady {
            version: "0.5.0".into(),
            install_command: "brew update && brew upgrade shep".into(),
        });

        assert_eq!(
            state.update_install_command,
            "brew update && brew upgrade shep"
        );
        let toast = state.toast.as_ref().expect("update toast");
        assert_eq!(
            toast.context,
            "detach, run `brew update && brew upgrade shep`, then restart this Shep session when ready"
        );
    }
