...
fn tab_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_tab_id) = args.first() else {
        eprintln!("usage: shep tab get <tab_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: shep tab get <tab_id>");
...
fn tab_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_tab_id) = args.first() else {
        eprintln!("usage: shep tab focus <tab_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: shep tab focus <tab_id>");
...
fn tab_rename(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: shep tab rename <tab_id> <label>");
        return Ok(2);
    }
...
fn tab_close(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_tab_id) = args.first() else {
        eprintln!("usage: shep tab close <tab_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: shep tab close <tab_id>");
...
fn print_tab_help() {
    eprintln!("shep tab commands:");
    eprintln!("  shep tab list [--workspace <workspace_id>]");
    eprintln!(
        "  shep tab create [--workspace <workspace_id>] [--cwd PATH] [--label TEXT] [--env KEY=VALUE] [--focus] [--no-focus]"
    );
    eprintln!("  shep tab get <tab_id>");
    eprintln!("  shep tab focus <tab_id>");
    eprintln!("  shep tab rename <tab_id> <label>");
    eprintln!("  shep tab close <tab_id>");
}
