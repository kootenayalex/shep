...
fn pane_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: shep pane get <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: shep pane get <pane_id>");
...
fn pane_current(args: &[String]) -> std::io::Result<i32> {
    let env_pane_id = std::env::var("SHEP_PANE_ID")
        .ok()
...
fn parse_pane_neighbor_args(args: &[String]) -> Result<PaneNeighborParams, String> {
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: shep pane neighbor --direction left|right|up|down [--pane ID|--current]".into(),
        );
    };
...
fn parse_pane_focus_args(args: &[String]) -> Result<PaneFocusDirectionParams, String> {
    let params = parse_pane_neighbor_args(args).map_err(|_| {
        "usage: shep pane focus --direction left|right|up|down [--pane ID|--current]".to_string()
    })?;
...
fn parse_pane_resize_args(args: &[String]) -> Result<PaneResizeParams, String> {
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: shep pane resize --direction left|right|up|down [--amount FLOAT] [--pane ID|--current]"
                .into(),
        );
    };
...
fn pane_rename(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: shep pane rename <pane_id> <label>|--clear");
        return Ok(2);
    };
    if args.len() < 2 {
        eprintln!("usage: shep pane rename <pane_id> <label>|--clear");
...
fn pane_read(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: shep pane read <pane_id> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
        return Ok(2);
    };
...
fn parse_pane_split_args(
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: shep pane split [<pane_id>|--pane ID|--current] --direction right|down [--ratio FLOAT] [--cwd PATH] [--env KEY=VALUE] [--focus] [--no-focus]"
                .into(),
        );
    };
...
fn pane_move_usage() -> String {
    "usage: shep pane move <pane_id> --tab <tab_id> --split right|down [--target-pane ID] [--ratio FLOAT] [--focus|--no-focus]\n       shep pane move <pane_id> --new-tab [--workspace ID] [--label TEXT] [--focus|--no-focus]\n       shep pane move <pane_id> --new-workspace [--label TEXT] [--tab-label TEXT] [--focus|--no-focus]"
        .into()
}
...
fn parse_pane_swap_args(args: &[String]) -> Result<PaneSwapParams, String> {
    ...
    match (directional, explicit) {
        ...
        _ => Err(
            "usage: shep pane swap --direction left|right|up|down [--pane ID|--current]\n       shep pane swap --source-pane ID --target-pane ID"
                .into(),
        ),
    }
}
...
fn pane_close(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: shep pane close <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: shep pane close <pane_id>");
...
fn pane_send_text(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: shep pane send-text <pane_id> <text>");
        return Ok(2);
    }
...
fn pane_send_keys(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: shep pane send-keys <pane_id> <key> [key ...]");
        return Ok(2);
    }
...
fn pane_report_agent(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: shep pane report-agent <pane_id> --source ID --agent LABEL --state idle|working|blocked|unknown [--message TEXT] [--custom-status TEXT] [--seq N] [--agent-session-id ID] [--agent-session-path PATH]");
        return Ok(2);
    };
...
fn pane_report_agent_session(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: shep pane report-agent-session <pane_id> --source ID --agent LABEL [--seq N] [--agent-session-id ID] [--agent-session-path PATH] [--session-start-source SOURCE]");
        return Ok(2);
    };
...
fn pane_release_agent(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: shep pane release-agent <pane_id> --source ID --agent LABEL [--seq N]");
        return Ok(2);
    };
...
fn pane_report_metadata(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: shep pane report-metadata <pane_id> --source ID [--agent LABEL] [--applies-to-source ID] [--title TEXT|--clear-title] [--display-agent TEXT|--clear-display-agent] [--custom-status TEXT|--clear-custom-status] [--state-label STATUS=TEXT] [--clear-state-labels] [--seq N] [--ttl-ms N]");
        return Ok(2);
    };
...
fn print_pane_help() {
    eprintln!("shep pane commands:");
    eprintln!("  shep pane list [--workspace <workspace_id>]");
    eprintln!("  shep pane current [--pane ID|--current]");
    eprintln!("  shep pane get <pane_id>");
    eprintln!("  shep pane layout [--pane ID|--current]");
    eprintln!("  shep pane process-info [--pane ID|--current]");
    eprintln!("  shep pane neighbor --direction left|right|up|down [--pane ID|--current]");
    eprintln!("  shep pane edges [--pane ID|--current]");
    eprintln!("  shep pane focus --direction left|right|up|down [--pane ID|--current]");
    eprintln!(
        "  shep pane resize --direction left|right|up|down [--amount FLOAT] [--pane ID|--current]"
    );
    eprintln!("  shep pane zoom [<pane_id>|--pane ID|--current] [--toggle|--on|--off]");
    eprintln!("  shep pane rename <pane_id> <label>|--clear");
    eprintln!("  shep pane read <pane_id> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
    eprintln!(
        "  shep pane split [<pane_id>|--pane ID|--current] --direction right|down [--ratio FLOAT] [--cwd PATH] [--env KEY=VALUE] [--focus] [--no-focus]"
    );
    eprintln!("  shep pane swap --direction left|right|up|down [--pane ID|--current]");
    eprintln!("  shep pane swap --source-pane ID --target-pane ID");
    eprintln!("  shep pane move <pane_id> --tab <tab_id> --split right|down [--target-pane ID] [--ratio FLOAT] [--focus|--no-focus]");
    eprintln!(
        "  shep pane move <pane_id> --new-tab [--workspace ID] [--label TEXT] [--focus|--no-focus]"
    );
    eprintln!("  shep pane move <pane_id> --new-workspace [--label TEXT] [--tab-label TEXT] [--focus|--no-focus]");
    ...
    eprintln!("  shep pane report-metadata <pane_id> --source ID [--agent LABEL] [--applies-to-source ID] [--title TEXT|--clear-title] [--display-agent TEXT|--clear-display-agent] [--custom-status TEXT|--clear-custom-status] [--state-label STATUS=TEXT] [--clear-state-labels] [--seq N] [--ttl-ms N]");
    eprintln!("  shep pane run <pane_id> <command>");
}
...
mod tests {
    ...
    fn parse_pane_swap_args_rejects_mixed_forms() {
        ...
        assert!(err.contains("usage: shep pane swap"));
    }
    ...
    fn parse_pane_move_args_rejects_target_pane_without_tab() {
        ...
        assert!(err.contains("usage: shep pane move"));
    }
