...
fn server_stop(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: shep server stop");
        return Ok(2);
    }
...
fn server_reload_config(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: shep server reload-config");
        return Ok(2);
    }
...
fn server_agent_manifests(args: &[String]) -> std::io::Result<i32> {
    let json = match args {
        ...
        _ => {
            eprintln!("usage: shep server agent-manifests [--json]");
            return Ok(2);
        }
    };
...
fn server_reload_agent_manifests(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: shep server reload-agent-manifests");
        return Ok(2);
    }
...
fn server_update_agent_manifests(args: &[String]) -> std::io::Result<i32> {
    let json = match args {
        ...
        [flag] if flag == "--json" => true,
        _ => {
            eprintln!("usage: shep server update-agent-manifests [--json]");
            return Ok(2);
        }
    };
...
fn server_live_handoff(args: &[String]) -> std::io::Result<i32> {
    let Some(params) = parse_live_handoff_params(args) else {
        eprintln!(
            "usage: shep server live-handoff [--import-exe <path>] [--expected-protocol <n>] [--expected-version <version>]"
        );
        return Ok(2);
    };
...
fn print_server_help() {
    eprintln!("shep server commands:");
    eprintln!("  shep server                run as headless server");
    eprintln!("  shep server stop           stop the running server via the API socket");
    eprintln!("  shep server live-handoff   hand off live panes to a new local server");
    eprintln!("  shep server reload-config  reload config.toml in the running server");
    eprintln!("  shep server agent-manifests [--json]  show agent detection manifest status");
    eprintln!(
        "  shep server update-agent-manifests [--json]  fetch and reload agent detection manifests"
    );
    eprintln!("  shep server reload-agent-manifests  reload agent detection manifests in the running server");
}
...
mod tests {
    ...
    fn live_handoff_params_parse_remote_update_fields() {
        let args = vec![
            ...
            "/home/me/.local/bin/shep".to_string(),
