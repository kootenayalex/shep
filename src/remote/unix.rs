...
const REMOTE_BINARY_ENV_VAR: &str = "SHEP_REMOTE_BINARY";
const SSH_CONTROL_SOCKET_NAME: &str = "ctl";
pub(crate) const REATTACH_COMMAND_ENV_VAR: &str = "SHEP_REATTACH_COMMAND";

pub(crate) const REMOTE_KEYBINDINGS_ENV_VAR: &str = "SHEP_REMOTE_KEYBINDINGS";
...
pub(crate) fn run_remote(remote: RemoteLaunch) -> io::Result<()> {
    ...
    let prepared_remote = prepare_remote_shep(&remote_ssh, remote.live_handoff)?;
    ensure_remote_server_ready(
        &remote_ssh,
        &prepared_remote.remote_shep,
...
pub(crate) fn run_remote_client_bridge() -> io::Result<()> {
    ...
    let stream = UnixStream::connect(&socket_path).map_err(|err| {
        io::Error::new(
            err.kind(),
            format!(
                "failed to connect to remote Shep client socket {}: {err}",
                socket_path.display()
            ),
        )
    })?;
...
fn ensure_remote_server_running() -> io::Result<()> {
    ...
    if crate::server::autodetect::is_server_listening() {
        ...
        return Err(io::Error::other(
            "remote shep server must restart before this bridge can attach; rerun `shep --remote` from an interactive terminal to approve stopping it",
        ));
    }
...
impl RemoteShep {
    fn for_platform(platform: RemotePlatform) -> Self {
        let install_suffix = ".local/bin/shep".to_string();
...
struct PreparedRemoteShep {
    remote_shep: RemoteShep,
...
impl RemoteSsh {
    ...
    fn install_shep(&self, remote_shep: &RemoteShep, source_path: &Path) -> io::Result<()> {
        let script = format!(
...
mv "$tmp" "$dest"
"#,
            install_suffix = remote_shep.install_suffix
        );
...
fn prepare_remote_shep(
    ssh: &RemoteSsh,
    live_handoff_enabled: bool,
) -> io::Result<PreparedRemoteShep> {
    let platform = detect_remote_platform(ssh)?;
    let remote_shep = RemoteShep::for_platform(platform);
    let override_binary = remote_binary_override_path()?;
    let remote_binary_candidates = remote_binary_candidates(ssh, &remote_shep)?;
    ...
    if override_binary.is_none() {
        for candidate in &remote_binary_candidates {
            if remote_binary_matches(ssh, candidate).unwrap_or(false) {
                return Ok(PreparedRemoteShep {
                    remote_shep: candidate.clone(),
                    installed_or_replaced: false,
                    stop_after_install_approved: false,
                });
            }
        }
        if remote_binary_matches(ssh, &remote_shep)? {
            return Ok(PreparedRemoteShep {
                remote_shep,
                installed_or_replaced: false,
                stop_after_install_approved: false,
    ...
    if let Some(status_probe_shep) = remote_binary_candidates.first().or_else(|| {
        remote_binary_exists(ssh, &remote_shep)
            .ok()
            .and_then(|exists| exists.then_some(&remote_shep))
    }) {
    ...
    confirm_remote_install(
        ssh.target(),
        &remote_shep,
        &install_source_description(&remote_shep.platform, override_binary.as_deref()),
    )?;
    let source = resolve_install_source(&remote_shep.platform, override_binary)?;
    let install_result = ssh.install_shep(&remote_shep, &source.path);
    source.cleanup();
    install_result?;

    if !remote_binary_matches(ssh, &remote_shep)? {
        return Err(io::Error::other(format!(
            "installed remote shep at {}, but it did not report version {}",
            remote_shep.shell_path,
            current_version()
        )));
    }
    warn_if_remote_bin_not_on_path(ssh)?;

    Ok(PreparedRemoteShep {
        remote_shep,
...
fn remote_binary_candidates(
    ssh: &RemoteSsh,
    remote_shep: &RemoteShep,
) -> io::Result<Vec<RemoteShep>> {
    let mut candidates = Vec::new();

    if let Some(path_candidate) = remote_binary_on_path_any(ssh, remote_shep)? {
        push_if_new_remote_binary_candidate(&mut candidates, path_candidate);
    }

    let output = ssh.sh_output(&known_remote_binary_candidate_script(&remote_shep.platform))?;
    if !output.status.success() {
        return Err(command_failed("remote binary discovery failed", &output));
    }
    let stdout = String::from_utf8_lossy(&output.stdout);
    for candidate in remote_sheps_from_path_discovery(remote_shep, &stdout) {
...
fn push_if_new_remote_binary_candidate(candidates: &mut Vec<RemoteShep>, candidate: RemoteShep) {
    if !candidates
        .iter()
...
if [ -n "$home" ]; then
    emit "$home/.local/bin/shep"
fi
"#,
    );
    if platform.os == "macos" {
        script.push_str(
            r#"    emit "/opt/homebrew/bin/shep"
    emit "/usr/local/bin/shep"
"#,
        );
    } else if platform.os == "linux" {
        script.push_str(
            r#"    emit "/home/linuxbrew/.linuxbrew/bin/shep"
"#,
        );
    }
    script.push_str(
        r#"if [ -n "$home" ]; then
    emit "$home/.local/share/mise/installs/shep/$version/bin/shep"
    emit "$home/.local/share/mise/installs/github-ogulcancelik-shep/$version/shep"
    emit "$home/.nix-profile/bin/shep"
fi
if [ -n "$user" ]; then
    emit "/etc/profiles/per-user/$user/bin/shep"
fi
emit "/nix/var/nix/profiles/default/bin/shep"
emit "/run/current-system/sw/bin/shep"
"#,
    );
...
fn remote_binary_on_path_any(
    ssh: &RemoteSsh,
    remote_shep: &RemoteShep,
) -> io::Result<Option<RemoteShep>> {
    let output = ssh.user_shell_output("command -v shep")?;
    if !output.status.success() {
        return Ok(None);
    }

    let stdout = String::from_utf8_lossy(&output.stdout);
    Ok(remote_shep_from_path_discovery(remote_shep, &stdout))
}

fn remote_sheps_from_path_discovery(remote_shep: &RemoteShep, stdout: &str) -> Vec<RemoteShep> {
    stdout
        .lines()
        .filter_map(|path| remote_shep_from_path(remote_shep, path))
        .collect()
}

fn remote_shep_from_path_discovery(remote_shep: &RemoteShep, stdout: &str) -> Option<RemoteShep> {
    stdout
        .lines()
        .find_map(|path| remote_shep_from_path(remote_shep, path))
}

fn remote_shep_from_path(remote_shep: &RemoteShep, path: &str) -> Option<RemoteShep> {
    let path = path.trim();
    if !path.starts_with('/') {
        return None;
    }
    ...
    Some(remote_shep.clone().with_shell_path(shell_quote(path)))
}

fn is_mise_shim_path(path: &str) -> bool {
    path.ends_with("/mise/shims/shep")
}

fn remote_binary_matches(ssh: &RemoteSsh, remote_shep: &RemoteShep) -> io::Result<bool> {
    let command = format!(
    ...
    Ok(version == format!("shep {}", current_version())
        && parse_client_status_json(status)
            .map(|status| status.protocol == CURRENT_PROTOCOL)
            .unwrap_or(false))
}

fn remote_binary_exists(ssh: &RemoteSsh, remote_shep: &RemoteShep) -> io::Result<bool> {
    let command = format!("test -x {}", remote_shep.shell_path);
...
fn install_source_description_for(
    ...
    if local_binary_can_seed_remote {
        "the current local shep binary".to_string()
    } else {
        format!(
            "the {} {} asset for {}",
            current_version(),
...
fn ensure_remote_server_ready(
    ...
    let status = remote_server_status(ssh, remote_shep)?;
    ...
    if live_handoff_enabled && live_handoff {
        match live_handoff_remote_server(ssh, remote_shep) {
            Ok(()) => return Ok(()),
            Err(err) => {
    ...
    if stop_after_install_approved {
        stop_remote_server(ssh, remote_shep)?;
        return Ok(());
    }
...
fn confirm_remote_install_with_running_server(
    ...
    let status = match remote_server_status(ssh, remote_shep) {
        Ok(status) => status,
        Err(err) => {
            if !io::stdin().is_terminal() {
                return Err(io::Error::other(format!(
                    "could not inspect the running remote shep server on {target} before installing: {err}; run from an interactive terminal to approve updating the remote binary"
                )));
            }
            eprintln!(
                "could not inspect the running remote shep server on {target} before installing: {err}"
            );
            eprint!("continue installing the remote shep binary? [y/N] ");
            io::stderr().flush()?;
    ...
    if plan == RemoteInstallRunningServerPlan::KeepRunning {
        if io::stdin().is_terminal() {
            eprintln!("remote shep server on {target} is already compatible:");
            eprintln!("  server: v{}", version_label(version.as_deref()));
            eprintln!(
                "Shep will install {} without stopping the running remote server.",
                current_version()
            );
        }
        return Ok(false);
    ...
    if !io::stdin().is_terminal() {
        match plan {
            ...
            RemoteInstallRunningServerPlan::StopRequired(_) => {
                return Err(io::Error::other(format!(
                    "remote shep server on {target} is running v{}; run from an interactive terminal to approve stopping it for the update",
    ...
    if plan == RemoteInstallRunningServerPlan::LiveHandoff {
        eprintln!("remote shep server on {target} is currently running:");
        eprintln!("  server: v{}", version_label(version.as_deref()));
        eprintln!(
            "Shep will install {} and hand off live pane processes to the prepared server.",
            current_version()
        );
        return Ok(false);
    }
    ...
    eprintln!("  server: v{}", version_label(version.as_deref()));
    eprintln!(
        "To complete the remote update, Shep must stop the running remote server after installing."
    );
...
fn remote_server_status(
    ...
    let command = format!("{} status server --json", remote_shep.shell_path);
    let output = ssh.sh_output(&command)?;
    if !output.status.success() {
...
fn confirm_remote_server_stop(
    ...
    if !io::stdin().is_terminal() {
        if reason == RemoteServerRestartReason::ProtocolMismatch {
            return Err(io::Error::other(format!(
                "remote shep server on {target} must stop before this client can attach; run from an interactive terminal to approve stopping it"
            )));
        }

        eprintln!(
            "remote shep server on {target} is still running v{}; it will use {} after it restarts.",
    ...
    match reason {
        ...
        RemoteServerRestartReason::DaemonDetachMissing => {
            eprintln!(
                "the remote server was started by a shep build that may not survive SSH connection loss. restart it so network drops disconnect only this client."
            );
        }
        RemoteServerRestartReason::BinaryUpdated => {
            eprintln!(
                "the remote shep binary was installed or replaced. restart the remote server so it uses the prepared binary."
            );
        }
        RemoteServerRestartReason::VersionMismatch => {
            eprintln!(
                "the remote server is still running a different shep version. restart it so it uses the prepared binary."
            );
        }
    }
    ...
    if reason == RemoteServerRestartReason::ProtocolMismatch {
        return Err(io::Error::new(
            io::ErrorKind::Interrupted,
            "remote shep server stop cancelled",
        ));
    }
...
fn live_handoff_remote_server(ssh: &RemoteSsh, remote_shep: &RemoteShep) -> io::Result<()> {
    let command = format!(
    ...
    eprintln!(
        "handed off the remote shep server on {}; reconnecting to the prepared server.",
        ssh.target()
    );
    Ok(())
}

fn stop_remote_server(ssh: &RemoteSsh, remote_shep: &RemoteShep) -> io::Result<()> {
    let command = format!("{} server stop", remote_shep.shell_path);
    let output = ssh.sh_output(&command)?;
    if !output.status.success() {
        return Err(command_failed("remote server stop failed", &output));
    }

    wait_for_remote_server_shutdown(ssh, remote_shep)?;
    eprintln!(
        "stopped the remote shep server on {}; it will restart when the remote client bridge attaches.",
        ssh.target()
    );
    Ok(())
}

fn wait_for_remote_server_shutdown(ssh: &RemoteSsh, remote_shep: &RemoteShep) -> io::Result<()> {
    let deadline = Instant::now() + REMOTE_SERVER_SHUTDOWN_CONFIRM_TIMEOUT;
    loop {
        if remote_server_status(ssh, remote_shep)? == RemoteServerStatus::NotRunning {
            return Ok(());
        }
        if Instant::now() >= deadline {
            return Err(io::Error::new(
                ...
                format!(
                    "shutdown was requested, but the old remote shep server on {target} is still responding after {} seconds",
...
fn warn_if_remote_bin_not_on_path(ssh: &RemoteSsh) -> io::Result<()> {
    ...
    eprintln!(
        "shep: installed remote binary to ~/.local/bin/shep, but the remote shell does not resolve `shep` to that path"
    );
    Ok(())
}
...
fn remote_shell_resolves_managed_install(stdout: &str) -> bool {
    stdout
        ...
        .next()
        .map(str::trim)
        .is_some_and(|path| path.ends_with("/.local/bin/shep"))
}
...
fn preview_assets_for_build<'a>(
    ...
    let build = manifest.builds.get(build_id).ok_or_else(|| {
        io::Error::other(format!(
            "preview manifest no longer includes build {build_id}; run `shep update` locally or set {REMOTE_BINARY_ENV_VAR}=target/release/shep"
        ))
    })?;
...
fn remote_release_asset(asset_key: &str) -> io::Result<RemoteReleaseAsset> {
    if crate::build_info::is_preview() {
        let build_id = crate::build_info::build_id().ok_or_else(|| {
            io::Error::other("preview client has no build id; set SHEP_REMOTE_BINARY or install Shep on the remote manually")
        })?;
        ...
        if protocol != CURRENT_PROTOCOL {
            return Err(io::Error::other(format!(
                "preview manifest has build {build_id} protocol {protocol}, but this client needs protocol {CURRENT_PROTOCOL}; set {REMOTE_BINARY_ENV_VAR}=target/release/shep or install a matching Shep on the remote host manually"
            )));
        }
    ...
    let release = manifest.release_for_version(&current_version).ok_or_else(|| {
        io::Error::other(format!(
            "release manifest does not include shep {current_version}; build shep for {} or install it there manually",
            asset_key
        ))
    })?;
    if let Some(protocol) = release.protocol {
        if protocol != CURRENT_PROTOCOL {
            return Err(io::Error::other(format!(
                "release manifest has shep {current_version} protocol {protocol}, but this client needs protocol {CURRENT_PROTOCOL}; set {REMOTE_BINARY_ENV_VAR}=target/release/shep or install a matching shep on the remote host manually"
            )));
        }
    }
    release
        ...
        .ok_or_else(|| {
            io::Error::other(format!(
                "no {asset_key} binary in the release manifest for shep {current_version}"
            ))
        })
}
...
fn private_download_dir(asset_key: &str) -> io::Result<PathBuf> {
    ...
    Err(io::Error::new(
        ...
        "failed to create private shep remote download directory",
    ))
}
...
fn confirm_remote_install(
    ...
    if !io::stdin().is_terminal() {
        return Err(io::Error::other(format!(
            "matching remote shep {} is not installed at {}; run from an interactive terminal to approve installation",
            current_version(),
            remote_shep.shell_path
        )));
    }
    ...
    eprintln!(
        "matching shep {} is not installed on {target} for {}.",
        current_version(),
        remote_shep.platform.asset_key()
    );
    eprint!(
        "Install {} to {}? [Y/n] ",
        source_description, remote_shep.shell_path
    );
    io::stderr().flush()?;
    ...
    if answer == "n" || answer == "no" {
        return Err(io::Error::new(
            io::ErrorKind::Interrupted,
            "remote shep installation cancelled",
        ));
    }
...
fn remote_bridge_command(remote_shep: &RemoteShep, session_name: &str) -> String {
    let mut command = format!("exec {}", remote_shep.shell_path);
...
fn reattach_command(
    ...
    let program = if program.is_empty() { "shep" } else { program };
                ...
                match listener.accept() {
                    Ok((stream, _addr)) => {
                        if let Err(err) = stream.set_nonblocking(false) {
                            eprintln!("shep: remote bridge failed to prepare client socket: {err}");
                            continue;
                        }
                        if let Err(err) = bridge_connection(
                            ...
                            eprintln!("shep: remote bridge failed: {err}");
                        }
                    }
                    ...
                    Err(err) => {
                        eprintln!("shep: remote bridge listener failed: {err}");
                        break;
                    }
                }
            }
...
/// from pre-planting a symlink or world-writable file that shep would write
...
fn private_ssh_config_dir() -> io::Result<PathBuf> {
    ...
    for base in bases {
        for attempt in 0..100 {
            let dir = base.join(format!("shep-ssh-{}-{attempt}", std::process::id()));
    ...
    Err(last_error.unwrap_or_else(|| {
        io::Error::new(
            io::ErrorKind::AlreadyExists,
            "failed to create private shep ssh config directory",
        )
    }))
}
...
/// shep's fallback would wrongly take effect.
...
/// an explicit `0` to disable it). Shep's keepalive values apply only when
/// the user has none.
...
fn bridge_connection(
    ...
    command
        .arg("-T")
        .arg(target)
        .arg(remote_bridge_command(remote_shep, session_name));
    command
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::inherit());
...
fn run_client_process(
    ...
    let status = Command::new(exe)
        ...
        .env("SHEP_RENDER_ENCODING", "terminal-ansi")
...
fn local_forward_socket_path(target: &str, session_name: &str) -> PathBuf {
    ...
    let readable = tmpdir.join(format!(
        "shep-remote-{pid}-{target_clean}-{session_clean}.sock"
    ));
    ...
    let short_name = format!("shep-r-{pid}-{target_prefix}-{hash}.sock");
...
mod tests {
    ...
    fn bridge_socket_is_user_only() {
        ...
        let socket = std::env::temp_dir().join(format!(
            "shep-bridge-permissions-test-{}.sock",
            std::process::id()
        ));
        let remote_shep = RemoteShep::for_platform(RemotePlatform {
            os: "linux",
            arch: "x86_64",
        });
    ...
    fn managed_ssh_config_includes_user_config_then_fallback() {
        ...
        // shep's fallback transport settings are present...
        assert!(
        ...
        if let Some(home) = std::env::var_os("HOME") {
            ...
            if user_config.is_file() {
                ...
                assert!(
                    ...
                    "user config must be Included before shep's fallback: {contents}"
                );
            }
        }
    ...
    fn extract_remote_args_removes_space_form() {
        ...
        assert_eq!(cleaned, vec!["shep", "--help"]);
    ...
    fn extract_remote_args_removes_equals_form() {
        let args = vec!["shep".into(), "--remote=user@host".into()];
        let (cleaned, remote) = extract_remote_args(&args).unwrap();
        assert_eq!(cleaned, vec!["shep"]);
        let remote = remote.unwrap();
    ...
    fn extract_remote_args_accepts_explicit_handoff() {
        let args = vec!["shep".into(), "--remote=dev".into(), "--handoff".into()];
    ...
    fn extract_remote_args_preserves_handoff_without_remote() {
        let args = vec!["shep".into(), "update".into(), "--handoff".into()];
    ...
    fn extract_remote_args_rejects_remote_keybindings_without_remote() {
        let args = vec!["shep".into(), "--remote-keybindings=server".into()];
    ...
    fn extract_remote_args_requires_value() {
        let args = vec!["shep".into(), "--remote".into()];
        let err = extract_remote_args(&args).unwrap_err();
    ...
    fn extract_remote_args_rejects_duplicate_values() {
        let args = vec!["shep".into(), "--remote=dev".into(), "--remote=prod".into()];
        let err = extract_remote_args(&args).unwrap_err();
    ...
    fn extract_remote_args_rejects_option_like_target() {
        let args = vec!["shep".into(), "--remote".into(), "-oProxyCommand=x".into()];
        let err = extract_remote_args(&args).unwrap_err();
    ...
    fn reattach_command_includes_remote_and_session() {
        assert_eq!(
            ...
            "target/release/shep --remote user@host --session work"
        );
        assert_eq!(
            reattach_command(
                "shep",
        ...
        assert_eq!(
            ...
            "shep --remote host --remote-keybindings server"
        );
        assert_eq!(
            reattach_command(
                "shep",
    ...
    fn remote_bridge_command_uses_installed_binary() {
        ...
        assert_eq!(
            remote_bridge_command(&remote_shep, crate::session::DEFAULT_SESSION_NAME),
            "exec \"$HOME/.local/bin/shep\" remote-client-bridge"
        );
    }
    ...
    fn remote_path_discovery_uses_path_binary() {
        ...
        let remote_shep =
            remote_shep_from_path_discovery(&remote_shep, "/usr/bin/shep\n").expect("path binary");

        assert_eq!(
            remote_bridge_command(&remote_shep, crate::session::DEFAULT_SESSION_NAME),
            "exec /usr/bin/shep remote-client-bridge"
        );
    }
    ...
    fn remote_path_discovery_quotes_discovered_binary() {
        ...
        let remote_shep = remote_shep_from_path_discovery(&remote_shep, "/opt/shep bin/shep\n")
            .expect("path binary");

        assert_eq!(
            remote_bridge_command(&remote_shep, crate::session::DEFAULT_SESSION_NAME),
            "exec '/opt/shep bin/shep' remote-client-bridge"
        );
    }
    ...
    fn remote_path_discovery_uses_macos_path_binary() {
        ...
        let remote_shep = remote_shep_from_path_discovery(&remote_shep, "/opt/homebrew/bin/shep\n")
            .expect("path binary");

        assert_eq!(
            remote_bridge_command(&remote_shep, crate::session::DEFAULT_SESSION_NAME),
            "exec /opt/homebrew/bin/shep remote-client-bridge"
        );
        assert_eq!(remote_shep.platform.asset_key(), "macos-aarch64");
    }
    ...
    fn remote_path_discovery_reads_multiple_absolute_paths() {
        ...
        let candidates = remote_sheps_from_path_discovery(
            &remote_shep,
            "/usr/bin/shep\nbin/shep\n /opt/shep bin/shep\n",
        );

        assert_eq!(candidates.len(), 2);
        assert_eq!(candidates[0].shell_path, "/usr/bin/shep");
        assert_eq!(candidates[1].shell_path, "'/opt/shep bin/shep'");
    }
    ...
    fn remote_path_discovery_ignores_mise_shims() {
        ...
        let candidates = remote_sheps_from_path_discovery(
            &remote_shep,
            "/home/can/.local/share/mise/shims/shep\n/home/can/.local/share/mise/installs/shep/0.7.1/bin/shep\n",
        );
    ...
    fn known_remote_binary_candidate_script_includes_mise_and_nix_paths() {
        ...
        assert!(script.contains("emit \"$home/.local/bin/shep\""));
        assert!(!script.contains("mise/shims/shep"));
        assert!(script.contains(&format!("version={}", shell_quote(&current_version()))));
        assert!(script.contains("emit \"$home/.local/share/mise/installs/shep/$version/bin/shep\""));
        assert!(script.contains(
            "emit \"$home/.local/share/mise/installs/github-ogulcancelik-shep/$version/shep\""
        ));
        assert!(script.contains("emit \"$home/.nix-profile/bin/shep\""));
        assert!(script.contains("emit \"/etc/profiles/per-user/$user/bin/shep\""));
        assert!(script.contains("emit \"/run/current-system/sw/bin/shep\""));
        assert!(script.contains("emit \"/home/linuxbrew/.linuxbrew/bin/shep\""));
        assert!(!script.contains("emit \"/opt/homebrew/bin/shep\""));
    }
    ...
    fn known_remote_binary_candidate_script_includes_macos_homebrew_paths() {
        ...
        assert!(script.contains("emit \"/opt/homebrew/bin/shep\""));
        assert!(script.contains("emit \"/usr/local/bin/shep\""));
        assert!(!script.contains("emit \"/home/linuxbrew/.linuxbrew/bin/shep\""));
    }
    ...
    fn remote_path_discovery_quotes_single_quotes_in_discovered_binary() {
        ...
        let remote_shep = remote_shep_from_path_discovery(&remote_shep, "/opt/shep's/bin/shep\n")
            .expect("path binary");

        assert_eq!(
            remote_bridge_command(&remote_shep, crate::session::DEFAULT_SESSION_NAME),
            "exec '/opt/shep'\\''s/bin/shep' remote-client-bridge"
        );
    }
    ...
    fn remote_path_discovery_ignores_relative_paths() {
        ...
        let remote_shep = remote_shep_from_path_discovery(&remote_shep, "bin/shep\n");

        assert!(remote_shep.is_none());
    }
    ...
    fn remote_path_discovery_ignores_empty_output() {
        ...
        let remote_shep = remote_shep_from_path_discovery(&remote_shep, "\n");
    ...
    fn remote_shell_path_warning_accepts_managed_install() {
        assert!(remote_shell_resolves_managed_install(
            "/home/can/.local/bin/shep\n"
        ));
        assert!(remote_shell_resolves_managed_install(
            "/Users/can/.local/bin/shep\n"
        ));
        assert!(!remote_shell_resolves_managed_install(
            "/usr/local/bin/shep\n"
        ));
    ...
    fn parse_client_status_json_reads_protocol() {
        assert_eq!(
            parse_client_status_json(r#"{"version":"x","protocol":8,"binary":"/bin/shep"}"#)
    ...
    fn install_source_description_uses_override_binary() {
        ...
        assert_eq!(
            install_source_description_for(&platform, Some(Path::new("/tmp/shep-aarch64")), false),
            "SHEP_REMOTE_BINARY (/tmp/shep-aarch64)"
        );
    }
    ...
    fn resolve_install_source_uses_override_binary_without_temporary_cleanup() {
        ...
        let source = resolve_install_source(&platform, Some(PathBuf::from("/tmp/shep-aarch64")))
            .expect("override source");
        assert_eq!(source.path, PathBuf::from("/tmp/shep-aarch64"));
    ...
    fn local_forward_socket_path_uses_readable_name_when_it_fits() {
        ...
        let filename = path
            ...
            .to_string();
        assert!(
            filename.starts_with("shep-remote-"),
    ...
    fn install_source_cleanup_removes_temporary_directory() {
        let dir = std::env::temp_dir().join(format!(
            "shep-install-source-cleanup-test-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&dir);
