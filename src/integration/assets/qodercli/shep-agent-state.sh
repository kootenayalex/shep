...
#
# Reports qodercli session identity to shep. Registered as a Command hook
# in ~/.qoder/settings.json by `shep integration install qodercli` and
...
action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/shep-qodercli-hook.XXXXXX")" || exit 0
