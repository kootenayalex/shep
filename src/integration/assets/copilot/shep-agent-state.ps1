...
if ([string]::IsNullOrWhiteSpace($env:SHEP_SOCKET_PATH)) { exit 0 }
...
& shep pane report-agent-session $env:SHEP_PANE_ID --source shep:copilot --agent copilot --agent-session-id $sessionId --seq $seq 2>$null | Out-Null
