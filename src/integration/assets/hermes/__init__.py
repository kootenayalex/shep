"""Hermes plugin installed by Shep to report agent lifecycle state."""

# SHEP_INTEGRATION_ID=hermes
# SHEP_INTEGRATION_VERSION=3
...
def _base_params() -> tuple[str, str] | None:
    if os.environ.get("SHEP_ENV") != "1":
        return None
    pane_id = os.environ.get("SHEP_PANE_ID", "").strip()
    socket_path = os.environ.get("SHEP_SOCKET_PATH", "").strip()
