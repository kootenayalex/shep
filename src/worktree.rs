...
";
    ...
    fn expand_tilde_path_uses_home_when_available() {
        assert_eq!(
            expand_tilde_path_from_env("~/.shep/worktrees", false, |key| match key {
                "HOME" => Some("/home/me".into()),
                _ => None,
            }),
            PathBuf::from("/home/me/.shep/worktrees")
        );
        assert_eq!(
    ...
    fn home_dir_uses_windows_profile_before_literal_home() {
        assert_eq!(
            home_dir_from_env(true, |key| match key {
                "HOME" => Some("~".into()),
                "USERPROFILE" => Some(r"C:\Users\shep".into()),
                _ => None,
            }),
            Ok(PathBuf::from(r"C:\Users\shep"))
        );
    }
    ...
    fn home_dir_uses_windows_drive_and_path_when_profile_is_missing() {
        assert_eq!(
            home_dir_from_env(true, |key| match key {
                ...
                "HOMEPATH" => Some(r"\Users\shep".into()),
                _ => None,
            }),
            Ok(PathBuf::from(r"C:\Users\shep"))
        );
    ...
    fn non_windows_tilde_expansion_keeps_windows_separator_literal() {
        assert_eq!(
            expand_tilde_path_from_env(r"~\.shep\worktrees", false, |key| match key {
                "HOME" => Some("/home/me".into()),
                _ => None,
            }),
            PathBuf::from(r"~\.shep\worktrees")
        );
    }
    ...
    fn windows_tilde_expansion_normalizes_separators() {
        ...
        let default_path = expand_tilde_path_from_env("~/.shep/worktrees", true, env);
        assert_eq!(
            default_path,
            PathBuf::from(r"C:\Users\shep\.shep\worktrees")
        );
        assert_eq!(
            default_path.display().to_string(),
            r"C:\Users\shep\.shep\worktrees"
        );
        assert_eq!(
            expand_tilde_path_from_env(r"~\.shep\worktrees", true, env),
            PathBuf::from(r"C:\Users\shep\.shep\worktrees")
        );
    }
    ...
    fn default_checkout_path_appends_repo_and_branch_slug() {
        assert_eq!(
            default_checkout_path(
                Path::new("/home/me/.shep/worktrees"),
                "shep",
                "worktree/brave-river",
            ),
            PathBuf::from("/home/me/.shep/worktrees/shep/worktree-brave-river")
        );
    }
    ...
    fn dirty_remove_error_detection_matches_git_force_hint() {
        assert!(is_dirty_worktree_remove_error(
            "fatal: '/w/shep' contains modified or untracked files, use --force to delete it"
        ));
        assert!(!is_dirty_worktree_remove_error(
            "fatal: '/w/shep' is a missing but already registered worktree"
        ));
        assert!(!is_dirty_worktree_remove_error(
            "fatal: '/w/shep' contains a locked worktree, use --force only if you know why"
        ));
    }
    ...
    fn worktree_add_command_creates_new_branch_from_base() {
        let command = build_worktree_add_new_branch_command(
            Path::new("/repo/shep"),
            Path::new("/w/shep/worktree-brave-river"),
            "worktree/brave-river",
            "HEAD",
        );
        assert_eq!(command.program, "git");
