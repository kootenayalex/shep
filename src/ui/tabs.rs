...
fn tab_width(
    ...
    display_width_u16(&tab_chrome_label(ws, tab_idx, terminals))
        .saturating_add(4)
        .max(MIN_TAB_WIDTH)
}
...
fn layout_tab_hit_areas(
    ...
    for (idx, rect) in rects.iter_mut().enumerate().skip(scroll) {
        ...
        let desired = tab_width(ws, idx, terminals);
...
fn centered_tab_scroll(
    ...
    for scroll in 0..=ws.active_tab {
        let rects = layout_tab_hit_areas(ws, area, scroll, terminals);
...
fn max_tab_scroll(
    ...
    (0..ws.tabs.len())
        .find(|&scroll| {
            layout_tab_hit_areas(ws, area, scroll, terminals)
                .last()
...
pub(crate) fn compute_tab_bar_view(
    ...
    if !mouse_chrome {
        let max_scroll = max_tab_scroll(ws, area, terminals);
        let scroll = if follow_active {
            centered_tab_scroll(ws, area, terminals).min(max_scroll)
        } else {
            current_scroll.min(max_scroll)
        };
        return TabBarView {
            scroll,
            tab_hit_areas: layout_tab_hit_areas(ws, area, scroll, terminals),
    ...
    let all_tabs = layout_tab_hit_areas(ws, all_tabs_area, 0, terminals);
    ...
    let max_scroll = max_tab_scroll(ws, tab_area, terminals);
    let scroll = if follow_active {
        centered_tab_scroll(ws, tab_area, terminals).min(max_scroll)
    } else {
        current_scroll.min(max_scroll)
    };
    let tab_hit_areas = layout_tab_hit_areas(ws, tab_area, scroll, terminals);
...
pub(super) fn render_tab_bar(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    for (idx, _tab) in ws.tabs.iter().enumerate() {
        ...
        let style = if active {
            ...
            if ws.tab_is_auto_named(idx, &app.terminals) {
                base
            } else {
                base.add_modifier(Modifier::BOLD)
            }
        } else if ws.tab_is_auto_named(idx, &app.terminals) {
            Style::default()
                .fg(p.overlay0)
                .bg(p.surface0)
                .add_modifier(Modifier::DIM)
...
mod tests {
    ...
    fn tab_bar_marks_zoomed_tabs_without_renaming_them() {
        ...
        assert_eq!(
            app.workspaces[0]
                .tab_display_name(0, &std::collections::HashMap::new())
                .as_deref(),
            Some("1")
        );
        assert_eq!(
            app.workspaces[0]
                .tab_display_name(custom_tab, &std::collections::HashMap::new())
                .as_deref(),
            Some("test")
        );
    }
    ...
    fn zoom_marker_counts_toward_tab_width() {
        ...
        assert_eq!(tab_width(&ws, 0, &std::collections::HashMap::new()), 14);
    }

    #[test]
    fn tab_width_uses_display_width_for_cjk_labels() {
        let mut ws = Workspace::test_new("test");
        ws.tabs[0].set_custom_name("提交 shep 的反馈".into());

        assert_eq!(
            tab_width(&ws, 0, &std::collections::HashMap::new()),
            display_width_u16("提交 shep 的反馈") + 4
        );
    }
