...
pub fn is_reserved_native_state_source(source: &str, agent: &str) -> bool {
    matches!(
        ...
        ("shep:claude", "claude")
            | ("shep:codex", "codex")
            | ("shep:copilot", "copilot")
            | ("shep:devin", "devin")
            | ("shep:droid", "droid")
            | ("shep:qodercli", "qodercli")
            | ("shep:cursor", "cursor")
    )
}
...
pub fn plan(source: &str, agent: &str, session_ref: &AgentSessionRef) -> Option<AgentResumePlan> {
    ...
    let argv = match (source, agent, session_ref.kind) {
        ("shep:claude", "claude", AgentSessionRefKind::Id) => {
            vec![
                "claude".into(),
                "--resume".into(),
        ...
        ("shep:pi", "pi", AgentSessionRefKind::Path | AgentSessionRefKind::Id) => {
            vec!["pi".into(), "--session".into(), session_ref.value.clone()]
        }
        ("shep:omp", "omp", AgentSessionRefKind::Path | AgentSessionRefKind::Id) => {
        ...
        ("shep:hermes", "hermes", AgentSessionRefKind::Id) => {
            vec![
                "hermes".into(),
                "--resume".into(),
                session_ref.value.clone(),
        ...
        ("shep:opencode", "opencode", AgentSessionRefKind::Id) => {
            vec![
                "opencode".into(),
                "--session".into(),
                session_ref.value.clone(),
        ...
        ("shep:qodercli", "qodercli", AgentSessionRefKind::Id) => {
            vec![
                "qodercli".into(),
                "--resume".into(),
                session_ref.value.clone(),
        ...
        ("shep:kilo", "kilo", AgentSessionRefKind::Id) => {
            vec!["kilo".into(), "--session".into(), session_ref.value.clone()]
        }
        ("shep:cursor", "cursor", AgentSessionRefKind::Id) => {
            vec![
                "cursor-agent".into(),
                "--resume".into(),
                session_ref.value.clone(),
...
mod tests {
    ...
    fn native_state_reservation_excludes_full_lifecycle_sources() {
        assert!(is_reserved_native_state_source("shep:claude", "claude"));
        assert!(is_reserved_native_state_source("shep:codex", "codex"));
        assert!(is_reserved_native_state_source("shep:devin", "devin"));
        assert!(!is_reserved_native_state_source("shep:kimi", "kimi"));
    ...
    fn report_ref_prefers_pi_and_omp_paths_and_validates_values() {
        ...
        assert!(session_ref_from_report("shep:pi", "pi", Some("bad\nid".into()), None).is_none());
        assert!(
            session_ref_from_report("shep:pi", "pi", None, Some("relative.jsonl".into())).is_none()
        );
        ...
        let session_ref =
            session_ref_from_report("shep:omp", "omp", Some("omp-id".into()), None).unwrap();
        ...
        assert_eq!(session_ref.value, "omp-id");
        assert!(
            session_ref_from_report("shep:omp", "omp", None, Some("relative.jsonl".into()))
                .is_none()
        );

        assert!(
            session_ref_from_report("shep:claude", "claude", None, Some(claude_session)).is_none()
        );

        let session_ref =
            session_ref_from_report("shep:copilot", "copilot", Some("copilot-id".into()), None)
                .unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        assert_eq!(session_ref.value, "copilot-id");
        assert!(
            session_ref_from_report("shep:copilot", "copilot", None, Some(copilot_session))
                .is_none()
        );

        let session_ref =
            session_ref_from_report("shep:devin", "devin", Some("devin-id".into()), None).unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        assert_eq!(session_ref.value, "devin-id");

        let session_ref =
            session_ref_from_report("shep:droid", "droid", Some("droid-id".into()), None).unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        ...
        let session_ref =
            session_ref_from_report("shep:kimi", "kimi", Some("kimi-id".into()), None).unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        ...
        let session_ref =
            session_ref_from_report("shep:kilo", "kilo", Some("kilo-id".into()), None).unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        assert_eq!(session_ref.value, "kilo-id");

        let session_ref =
            session_ref_from_report("shep:qodercli", "qodercli", Some("qoder-id".into()), None)
                .unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
    ...
    fn ids_are_data_not_shell_text() {
        let id = "abc; rm -rf /";
        let codex_plan = plan("shep:codex", "codex", &AgentSessionRef::id(id).unwrap()).unwrap();
        assert_eq!(codex_plan.argv, vec!["codex", "resume", id]);

        let copilot_plan =
            plan("shep:copilot", "copilot", &AgentSessionRef::id(id).unwrap()).unwrap();
        assert_eq!(copilot_plan.argv, vec!["copilot", "--resume=abc; rm -rf /"]);

        let devin_plan = plan("shep:devin", "devin", &AgentSessionRef::id(id).unwrap()).unwrap();
